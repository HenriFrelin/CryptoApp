...
open class EmojiRateView: UIView {
    ...
    /// Current line color.
    @IBInspectable open var rateColor: UIColor = UIColor.init(red: 100 / 256, green: 43 / 256, blue: 250 / 255, alpha: 1.0) {
        didSet {
            redraw()
        }
    }
    ...
    fileprivate func configure() {
        ...
        let custom = UIColor(red: 100/255, green: 43/255, blue: 250/255, alpha: 1)
    ...
    fileprivate func facePathWithRect(_ rect: CGRect) -> UIBezierPath {
        ...
        let facePath = UIBezierPath(ovalIn: rect.inset(by: UIEdgeInsets.init(top: margin, left: margin, bottom: margin, right: margin)))
        return facePath
    }
